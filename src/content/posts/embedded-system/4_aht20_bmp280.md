...
Adafruit_AHTX0 aht;
Adafruit_BMP280 bmp; // I2C 통신 사용

float base_pressure = 1013.25; // 초기 지상 기압 기준점
```
...
void setup() {
  Serial.begin(115200);
  Wire.begin(21, 22); // 하드웨어 I2C 핀 지정
  ...
  // BMP280 주소는 모듈에 따라 0x76 또는 0x77일 수 있음
...
void loop() {
  ...
  float pressure = bmp.readPressure() / 100.0; // hPa 변환
  float altitude = bmp.readAltitude(base_pressure); // 상대 고도 계산
